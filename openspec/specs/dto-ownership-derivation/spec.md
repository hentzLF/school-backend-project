# Spec: DTO Ownership Derivation

## Purpose

Defines the contract for creation DTOs to prevent caller-supplied ownership fields. Ownership identifiers (ClientProfileId, UserProfileId, ReviewerProfileId) MUST be omitted from request bodies and derived server-side from the authenticated caller's JWT claims.
