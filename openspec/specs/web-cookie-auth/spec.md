...
The system SHALL configure cookie-based authentication in AgriMarket.Web using `CookieAuthenticationDefaults.AuthenticationScheme`. The login path SHALL be `/Account/Login`. The access denied path SHALL be `/Account/AccessDenied`. Cookie SHALL be HTTP-only and secure in production.

#### Scenario: Unauthenticated user accesses protected page
- **WHEN** an unauthenticated user navigates to any Admin area page
- **THEN** the system redirects to `/Account/Login`
...
- **THEN** the system redirects to `/Account/AccessDenied`
